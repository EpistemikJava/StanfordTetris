...
/**
 * JTetris presents a tetris game in a window. It handles the GUI and the animation. The Piece and Board classes handle the
 * lower-level computations. This code is provided in finished form for the students. See Tetris-Architecture.html for an
 * overview.
 * 
 * Implementation notes: -The "currentPiece" points to a piece that is currently falling, or is null when there is no piece.
 * -tick() moves the current piece -a timer object calls tick(DOWN) periodically -keystrokes call tick with LEFT, RIGHT, etc.
 * -Board.undo() is used to remove the piece from its old position and then Board.place() is used to install the piece in its
 * new position.
 * 
...
public class JTetris extends JComponent {
    ...
    /**
     * Given a piece, tries to install that piece into the board and set it to be the current piece. Does the necessary
     * repaints. If the placement is not possible, then the placement is undone, and the board is not changed.<br>
    ...
    /**
     * Figures a new position for the current piece based on the given verb (LEFT, RIGHT, ...). The board should be in the
     * committed state -- i.e. the piece should not be in the board at the moment. This is necessary so dropHeight() may be
     * called without the piece "hitting itself" on the way down. <br>
     * Sets the ivars newX, newY, and newPiece to hold what it thinks the new piece position should be. (Storing an intermediate
     * result like that in ivars is a little tacky.)
     * 
    ...
    /**
     * Called to change the position of the current piece. Each key press call this once with the verbs LEFT RIGHT ROTATE DROP
     * for the user moves, and the timer calls it with the verb DOWN to move the piece down one square. <br>
     * Before this is called, the piece is at some location in the board. This advances the piece to be at its next location. <br>
    ...
    /**
     * Draws the current board with a 1 pixel border around the whole thing. Uses the pixel helpers above to map board coords to
     * pixel coords. Draws rows that are filled all the way across in green.
     */
        ...
        for( x = 0; x < $bWidth; x++ ) {
            ...
            for( y = 0; y < $yHeight; y++ ) {
                if( board.getGrid(x, y) ) {
                    ...
                    if( $filled ) page.setColor(Color.green);

                    page.fillRect($left + 1, yPixel(y) + 1, $dx, $dy); // +1 to leave a white border

                    if( $filled ) page.setColor(Color.black);
                }
            }// for
        }// for
    }// paintComponent()
    ...
    /**
     * Creates the panel of UI controls. This code is very repetitive -- the GUI/XML extensions in Java 1.4 should make this
     * sort of ugly code less necessary.
    ...
    public static void main(String[] args) {
        ...
        if( args.length != 0 && args[0].equals("test") ) $myTetris.testMode = true;
    ...
    static final int ROTATE = 0, LEFT = 1, RIGHT = 2, DROP = 3, DOWN = 4;
