...
/**
 * Represents a Tetris board -- essentially a 2-d grid of booleans. Supports tetris pieces and row clearning. Has an "undo"
 * feature that allows clients to add and remove pieces efficiently. Does not do any drawing or have any idea of pixels. Intead,
 * just represents the abtsract 2-d board. See Tetris-Architecture.html for an overview.
 * 
...
final class Board {
    ...
    /**
     * Given a piece and an x, returns the y value where the piece would come to rest if it were dropped straight down at that
     * x. <br>
    ...
    /**
     * Attempts to add the body of a piece to the board. Copies the piece blocks into the board grid. Returns PLACE_OK for a
     * regular placement, or PLACE_ROW_FILLED for a regular placement that causes at least one row to be filled.
     * <p>
     * Error cases: If part of the piece would fall out of bounds, the placement does not change the board at all, and
     * PLACE_OUT_BOUNDS is returned. If the placement is "bad" --interfering with existing blocks in the grid -- then the
     * placement is halted partially complete and PLACE_BAD is returned. An undo() will remove the bad placement.
     * 
    ...
    int place(Piece piece, int x, int y) {
        ...
        for( int i = 0; i < $bodyAr.length; i++ ) {
            ...
            if( widths[$putY] >= width ) $rowFilled = true;
        }
     ...
     * <b>Implementation:</b> This is complicated. Ideally, you want to copy each row down to its correct location in one pass.
    ...
    /**
     * If a place() happens, optionally followed by a clearRows(), a subsequent undo() reverts the board to its state before the
     * place(). If the conditions for undo() are not met, such as calling undo() twice in a row, then the second undo() does
     * nothing. See the overview docs.
     */
    void undo() {
        if( !committed ) {
            int[] $tempAr;
    ...
    */
    ...
    static final int PLACE_OK = 0, PLACE_ROW_FILLED = 1, PLACE_OUT_BOUNDS = 2, PLACE_BAD = 3;
