...
/**
 * An immutable representation of a tetris piece in a particular rotation. Each piece is defined by the blocks that make up its
 * body. See the Tetris-Architecture.html for an overview. <br>
...
final class Piece {
    ...
    /**
     * Defines a new piece given the Points that make up its body. Makes its own copy of the array and the Points inside it.
     * Does not set up the rotations.
     * 
     * This constructor is PRIVATE -- if a client wants a piece object, they must use Piece.getPieces().
     */
    ...
    /**
     * @return a pointer to the piece's body. The caller should not modify this array.
     */
    Point[] getBody() {
        return body;
    }
    ...
    /**
     * For each x value across the piece, the skirt gives the lowest y value in the body. This useful for computing where the
     * piece will land. The caller should not modify this array.
    ...
    /**
     * Implementation: The Piece class pre-computes all the rotations once. This method just hops from one pre-computed rotation
    ...
    /**
     * Interestingly, this is not the same as having exactly the same body arrays, since the points may not be in the same order
     * in the bodies. Used internally to detect if two rotations are effectively the same.
    ...
    public boolean equals(Object obj) {
        ...
        if( ($pce).body.length != this.body.length ) return false;
    ...
    /**
     * Returns an array containing the first rotation of each of the 7 standard tetris pieces. The next (counterclockwise)
     * rotation can be obtained from each piece with the {@link #nextRotation()} message. In this way, the client can iterate
     * through all the rotations until eventually getting back to the first rotation. (provided code)
    ...
    /**
     * Given the "first" rotation of a piece piece, computes all the other rotations and links them all together by their next
     * pointers. Returns the first piece. {@link #nextRotation()} relies on the next pointers to get from one rotation to the
     * next. Internally, uses Piece.equals() to detect when the rotations have gotten us back to the first piece.
     */
